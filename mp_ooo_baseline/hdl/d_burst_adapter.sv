...
// It has 3 states:
// 1. IDLE: waits for a read/write request from the cache
// 2. READ: sends 4 read requests to the DRAM and collects the data
// 3. WRITE: sends 4 write requests to the DRAM

module d_burst_adapter
import rv32i_types::*;
(
    input  logic clk,
    input  logic rst,
...
always_comb begin
    ...
    start_read = (burststate == idle) && (cache_read != 1'b0) && d_cache_chosen;
    ...
    unique case (burststate)
      idle: begin
        if (start_read) begin
            ...
            burstcnt_nxt   = 2'b00;
            burstreg_next  = '0;
        end else if ((cache_write != 1'b0)) begin
            burststate_nxt = write;
            addr_reg_nxt   = cache_addr;
            bmem_addr      = cache_addr;
            burstcnt_nxt   = 2'b00;
        end
      ...
      write: begin
        bmem_write = 1'b1;
        case (burstcnt)
            2'b00: bmem_wdata = cache_wdata[BURST_SIZE-1:0];
            2'b01: bmem_wdata = cache_wdata[2*BURST_SIZE-1:BURST_SIZE];
            2'b10: bmem_wdata = cache_wdata[3*BURST_SIZE-1:2*BURST_SIZE];
            2'b11: bmem_wdata = cache_wdata[4*BURST_SIZE-1:3*BURST_SIZE];
        endcase

        if (burstcnt == 2'b11) begin
            cache_resp     = 1'b1;
            burststate_nxt = idle;
            burstcnt_nxt   = 2'b00;
        end else begin
            burstcnt_nxt   = burstcnt + 2'b01;
        end
      end
    ...
    // Handle read data collection for both idle->read transition and read state
