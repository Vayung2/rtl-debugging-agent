...
(
    ...
    always_comb begin // decides what the next state should be 
        unique case (state)
            idle: begin
                if (op_load) begin // wait for a valid read or write request
        ...
        unique case (state)
            ...
            compare_tag: begin
                if (op_load && hit) begin
                    ufp_cacheline = data_dout[hit_index];
                    case (req_addr[4:2])
                        3'b000: ufp_rdata = ufp_cacheline[31:0];
                        3'b001: ufp_rdata = ufp_cacheline[63:32];
                        3'b010: ufp_rdata = ufp_cacheline[95:64];
                        3'b011: ufp_rdata = ufp_cacheline[127:96];
                        3'b100: ufp_rdata = ufp_cacheline[159:128];
                        3'b101: ufp_rdata = ufp_cacheline[191:160];
                        3'b110: ufp_rdata = ufp_cacheline[223:192];
                        3'b111: ufp_rdata = ufp_cacheline[255:224];
                    endcase
                    ufp_resp = 1'b1;
                    plru_web = 1'b0; 
                    plru_din = plru_nxt;
    ...
    always_ff @ (posedge clk) begin // advances the state to next state 
        if (rst || (branch_flush.valid && branch_flush.branch_taken && (branch_flush.branchtg_addr[31:5] != ufp_addr[31:5]))) begin
            state <= idle;
        end else begin
            state <= next_state;
        end
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            req_is_store  <= 1'b0;
            req_set_index <= '0;
        end else if (branch_flush.valid && branch_flush.branch_taken) begin
            req_addr      <= '0;
            req_rmask     <= '0;
            req_is_load   <= 1'b0;
            req_set_index <= '0;
        end else if (state == idle && live_is_load) begin
            req_addr      <= ufp_addr;
            req_rmask     <= ufp_rmask;
